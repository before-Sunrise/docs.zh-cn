...
* 从 3.0 版本开始支持 `IGNORE NULLS`，即是否在计算结果中忽略 NULL 值。如果不指定 `IGNORE NULLS`，默认会包含 NULL 值。比如，如果指定的当前行之前的第offset行的值为 NULL，则返回 NULL。如果指定了 `IGNORE NULLS`，则向前遍历offset行时会忽略NULL值。如果不存在offset个非NULL值，那么即使指定了 `IGNORE NULLS`，也会返回 NULL。
...
* 从 3.0 版本开始支持 `IGNORE NULLS`，即是否在计算结果中忽略 NULL 值。如果不指定 `IGNORE NULLS`，默认会包含 NULL 值。比如，如果指定的当前行之后第offset行的值为 NULL，则返回 NULL。如果指定了 `IGNORE NULLS`，则向后遍历offset行时会忽略NULL值。如果所有值都为 NULL，那么即使指定了 `IGNORE NULLS`，也会返回 NULL。
